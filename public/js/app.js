...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.continueSavedTournament =  function(id) {
  ...
  window.showIndividualModal = function () {
  ...
  window.confirmIndividualTournament = async function confirmIndividualTournament1() {
  ...
  window.confirmTeamTournament = async function confirmTeamTournament1() {
    const auth = window.auth;
    const db = window.db;
    const user = auth.currentUser;
    const tokenResult = await user.getIdTokenResult();
  ...
  window.goBackToMainFromTournament = function() {
  ...
  window.adjustScore = function(matchId, scoreField, value) {
  ...
  window.updateScore = function(matchId, scoreType, value) {
    for (let round of tournamentData.rounds) {
      const matchIndex = round.matches.findIndex(m => m.id === matchId);
      if (matchIndex !== -1) {
